...
void draw() { 
  ...
  for (int j = 0; j < f; j++) for (int i = 0; i < b[j].length; i++) {
    ...
    if (fC - aS < aL && pp[j][i][0]>0) { 
      ...
      if (pp[j][i][0]>1) { 
        val=pp[j][i][0]; 
        textt(""+pp[j][i][0], xt, yt+txv, bs, bs, bb, 40, C);
    ...
    if (fC-aS>aL||pp[j][i][0]>=0) {
      ...
      if (val>0) {
        rectt(x, y, bs, bs, 5, color(255-log(val)/log(2)*255/11, log(val)/log(2)*255/11, 0));
  ...
  if (dead>0) { 
    rectt(0, 0, w, h, 0, color(255, 100)); 
    textt("Gameover! Click to restart", 0, h/2, w, 50, bb, 30, C); 
...
void keyPressed() {
  ...
  if (dead>0)return;
  int k=keyCode, dy=k==UP?-1:(k==40?1:0), dx=k==37?-1:(k==39?1:0);
...
boolean gameover() { 
  ...
  boolean out=true; 
  for (int i=0; i<f; i++) if (go(dy[i], dx[i], false) != null) out=false;
  ;
  ; 
  pp=ppbk; 
  return out;
...
int[][] go(int dy, int dx, boolean ups) { 
  pp = new int[4][4][3]; 
  boolean mv = false; 
  int[][] bk = new int[4][4]; 
